...
interface CardModalProps {
  ...
  onSave: (
    ...
    imageUrl?: string,
    dueDate?: Date,
    opCode?: string // 🟢 NOVA PROPRIEDADE PARA A OP
  ) => void;
...
export const CardModal = ({ isOpen, onClose, onSave, editingCard, readOnly = false }: CardModalProps) => {
  ...
  const [opCode, setOpCode] = useState(''); // 🟢 NOVO ESTADO PARA OP
  ...
  useEffect(() => {
    if (editingCard) {
      ...
      setOpCode(editingCard.opCode || ''); // 🟢 CARREGA OP SE EDITAR
    } else {
      setTitle('');
      setDescription('');
      setPriority('medium');
      ...
      setTags([]);
      setOpCode(''); // 🟢 LIMPA OP SE FOR NOVO
    }
    setNewItemText('');
  }, [editingCard, isOpen]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (title.trim()) {
      onSave(
        ...
        imageUrl,
        dueDate,
        opCode.trim() || undefined // 🟢 ENVIA A OP
      );
      onClose();
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto animate-zoom-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-4">
          ...
          {/* 🟢 Número da OP (Novo Campo) */}
          <div className="space-y-2">
            <Label htmlFor="opCode">
              Número da OP <span className="text-destructive">*</span>
            </Label>
            <Input
              id="opCode"
              value={opCode}
              ...
              className="h-11 transition-all duration-200 focus:scale-[1.01]"
              disabled={readOnly} // Bloqueado
              required // Transformado em obrigatório para gestão
            />
          </div>
