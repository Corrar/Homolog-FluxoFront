...
export function Sidebar({ isCollapsed, toggleSidebar, onItemClick, isMobileMenu }: SidebarProps) {
  ...
  const has3DAccess = isAdmin || canAccess('producao_3d');
      ...
      >
        ...
        {!isCollapsed && (
          <>
            ...
            {showBadge && (
              <span className="flex items-center justify-center min-min-w-[24px] h-6 px-2 rounded-full bg-red-500 text-[11px] font-black text-white shadow-sm animate-in zoom-in duration-300 shrink-0">
                {formatBadgeCount(unreadCount)}
              </span>
            )}
          </>
              ...
              {showAdminGroup && (
                <NavGroup title="Gestão Admin" isCollapsed={isCollapsed} isMobileMenu={isMobileMenu}>
                  <div className={!isMobileMenu ? "px-2" : "flex flex-col gap-2.5 w-full"}>
                    ...
                    {isAdmin && renderLink("/settings", <Settings className={isMobileMenu ? "h-6 w-6" : "h-[22px] w-[22px]"} strokeWidth={2.2} />, "Configurações")}
            ...
            <>
              <NavGroup title="Fábrica 3D" isCollapsed={isCollapsed} isMobileMenu={isMobileMenu}>
                <div className={!isMobileMenu ? "px-2" : "flex flex-col gap-2.5 w-full"}>
                  ...
                  {renderLink("/producao-3d/producao", <Factory className={isMobileMenu ? "h-6 w-6 text-purple-500" : "h-[22px] w-[22px] text-purple-500"} strokeWidth={2.2} />, "Histórico de Produção")}
