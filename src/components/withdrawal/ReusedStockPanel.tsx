...
import { Plus, Trash2, Recycle, Loader2, AlertCircle, CheckCircle2 } from "lucide-react";
      ...
      <div className="relative">
        <Input
          ...
          className={cn(
            ...
            isValid && "border-emerald-500/50 pr-10 focus-visible:ring-[#facc15]"
          )}
        />
...
export const ReusedStockPanel = () => {
  ...
  // Bloqueio do botão: Todos têm de ser válidos e ter quantidade > 0
  const canSubmit = items.every(i => i.isValid && Number(i.quantidade) > 0);

  const submit = async () => {
    if (!canSubmit) return toast.error("Verifique se selecionou materiais válidos e as respetivas quantidades.");
    ...
    try {
      const payload = items.map(i => ({
        ...
        type: 'REAPROVEITAMENTO', // Indica ao backend que é reaproveitamento
        observation: 'Custo Zero' // Fica registado no Log da DB
      }));
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AVISO EM DESTAQUE AMARELO (Como pediste) */}
      ...
      <div className="space-y-4">
        {items.map((item, idx) => (
          <div key={item.id} className="grid grid-cols-1 md:grid-cols-[1fr_180px_120px_45px] gap-3 items-end animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="space-y-2">
              {idx === 0 && <Label className="text-foreground">Produto Resgatado</Label>}
              <AutocompleteInput
                placeholder="Busque o material resgatado..."
                value={item.produto}
              ...
              {!item.isValid && item.produto.length > 2 && (
                <span className="text-[10px] text-destructive font-bold uppercase animate-pulse">
                  Erro: Material não existe no estoque
                </span>
              )}
            </div>
        ...
        >
          <Plus className="size-4 mr-2" /> Adicionar outro material resgatado
        </Button>
      </div>
      ...
      <div className="pt-4 border-t border-white/5">
        ...
        >
          {isSubmitting ? (
            ...
            "Registrar Reaproveitamento no Estoque"
          )}
        </Button>
      </div>
    </div>
