...
export function useCards() {
  ...
  const { data: cards = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.data.map((task: any) => ({
        ...
        opCode: task.opCode // 🟢 Certifica-te de mapear o opCode que vem do backend
      }));
    },
    staleTime: 1000 * 60 * 5, 
  });
  ...
  const createMutation = useMutation({
    ...
    // 🟢 Trata o erro da OP se ela estiver fechada ou não existir
    onError: (error: any) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<CardData & { op_code?: string }> }) => 
    ...
    // 🟢 Trata o erro ao atualizar se a OP for inválida
    onError: (error: any) => {
      const msg = error.response?.data?.error || "Erro ao atualizar tarefa.";
      toast.error(msg);
    }
  });
  ...
  // 🟢 Adicionado opCode como parâmetro
  const addCard = (
    title: string, 
    description: string, 
    priority: Priority, 
    ...
    imageUrl?: string, 
    dueDate?: Date,
    opCode?: string // Novo parâmetro
  ) => {
    createMutation.mutate({
      title, description, priority, checklist, tags, imageUrl, dueDate,
      op_code: opCode, // Passado para o backend
  ...
  const updateCard = (
    ...
    updateMutation.mutate({ 
      id, 
      // 🟢 Envia o op_code no corpo da atualização
      data: { title, description, priority, checklist, tags, imageUrl, dueDate, op_code: opCode } 
    });
  };
  ...
  const duplicateCard = (id: string) => {
    ...
    if (cardToDuplicate) {
      ...
      addCard(
        ...
        cardToDuplicate.opCode // 🟢 Repassa a OP para a duplicação!
      );
    }
  };
