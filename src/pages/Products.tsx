...
const ProductCard = ({ 
  product, isPurchaseMode, inCart, isEditingThis, isJustAdded,
  onEdit, onDelete, onToggleCart, onOpenPriceDialog, 
  canEdit, canDelete, canEditPrice, canViewPrice, selectedTags, toggleFilterTag, isVisible, primaryColor 
}: any) => {
  ...
  const physicalStock = Number(product.stock?.quantity_on_hand) || 0;
  const reservedStock = Number(product.stock?.quantity_reserved) || 0;
      ...
      <div className={`flex items-end justify-between mt-auto pt-4 border-t relative z-10 ${isEditingThis ? "border-amber-200/50" : "border-slate-100/80 dark:border-slate-800"}`}>
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex flex-col">
            <span className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-0.5">Disponível</span>
            <div className={`flex items-baseline gap-1 ${isEditingThis ? "text-amber-600" : "text-emerald-600 dark:text-emerald-400"}`}>
              <span className="font-black text-xl sm:text-2xl leading-none tracking-tighter">
                {isVisible ? availableStock : "••••"}
              </span>
              <span className="text-[10px] sm:text-[11px] font-bold opacity-70 lowercase">{product.unit}</span>
            </div>
          </div>

          <div className="flex items-center gap-1">
            <span className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest">Físico:</span>
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">
              {isVisible ? physicalStock : "••••"} <span className="lowercase text-[9px]">{product.unit}</span>
            </span>
          </div>
        </div>
          ...
          <div className="flex flex-col items-end group/price">
            ...
            <div 
              onClick={(e) => { 
                if (canEditPrice) {
                  e.stopPropagation(); 
                  onOpenPriceDialog(product); 
                }
              }}
              className={`font-black flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-[12px] -mr-2 sm:-mr-3 transition-colors ${canEditPrice ? "cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95" : "cursor-default"} ${hasPrice ? "text-slate-900 dark:text-white text-[16px] sm:text-[18px] tracking-tighter" : `text-${primaryColor}-500 text-[14px]`}`}
            >
              {!hasPrice && canEditPrice && <Plus className="h-3.5 w-3.5" strokeWidth={3} />}
              {isVisible ? (hasPrice ? priceFormatted : (canEditPrice ? "Definir" : "R$ 0,00")) : "R$ ••••"}
              {canEditPrice && (
...
export default function Products() {
  ...
  const [skuToReactivate, setSkuToReactivate] = useState<string | null>(null);
  ...
  const canAddProduct = canAccess("produtos:add");
  const canEditProduct = canAccess("produtos:edit");
  const canDeleteProduct = canAccess("produtos:delete");
  const canEditPrice = canAccess("valores:edit");
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data && data.id) {
          setSessionAddedIds(prev => [data.id, ...prev]);
          setCurrentPage(1); 
      }
    ...
    onError: (error: any, variables: any) => {
      ...
      if (errorMsg.includes("arquivado") || errorMsg.includes("inativo")) {
        if (canAddProduct || canEditProduct) {
          ...
          toast.error("Este código SKU pertence a um produto inativo. Peça a um Administrador ou Almoxarife para reativá-lo.");
        }
      } else {
  ...
  const reactivateMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["products"] });
      toast.success("Produto reativado e pronto a usar!");
      
      if (data && data.product && data.product.id) {
          setSessionAddedIds(prev => [data.product.id, ...prev]);
          setCurrentPage(1);
      }
      
      setSkuToReactivate(null);
      resetForm();
      setShowMobileForm(false);
    },
  ...
  const updatePriceMutation = useMutation({
    mutationFn: async ({ id, price, salesPrice }: { id: string; price: number, salesPrice: number }) => {
      return await updateProductPrices(id, { unit_price: price, sales_price: salesPrice });
    },
    ...
    onError: (err: any, _new, context) => {
        if (context?.prev) queryClient.setQueryData(["products"], context.prev);
        const serverError = err.response?.data?.error || "Erro desconhecido ao atualizar preço.";
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (sessionAddedIds.length > 0) {
      result = [...result].sort((a: any, b: any) => {
          const idxA = sessionAddedIds.indexOf(a.id);
          const idxB = sessionAddedIds.indexOf(b.id);
    ...
    return result;
  }, [products, searchTerm, selectedTags, sessionAddedIds]);
  ...
  const { valorUsinagem, valorOutros, totalGeral } = useMemo(() => {
    let usinagem = 0;
    let outros = 0;

    if (!products) return { valorUsinagem: 0, valorOutros: 0, totalGeral: 0 };

    products.forEach((p: any) => {
      const preco = parseFloat(p.unit_price || "0");
      const qtd = parseFloat(p.stock?.quantity_on_hand || "0");
      const valorProduto = preco * qtd;

      const temTagUsinagem = p.tags && Array.isArray(p.tags) && p.tags.some((t: string) => t.toUpperCase() === "USINAGEM");

      if (temTagUsinagem) {
        usinagem += valorProduto;
      } else {
        outros += valorProduto;
      }
    });
    ...
    return { 
      valorUsinagem: usinagem, 
      valorOutros: outros, 
      totalGeral: usinagem + outros 
    };
  }, [products]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/50 dark:bg-slate-950 font-sans pb-28 sm:pb-24 transition-colors duration-500">
      ...
      <div className={`pt-10 pb-20 px-4 sm:px-6 relative z-10 shrink-0 rounded-b-[40px] shadow-[0_10px_40px_rgba(0,0,0,0.1)] overflow-hidden transition-all duration-700 ${isPurchaseMode ? 'bg-gradient-to-br from-violet-600 via-indigo-600 to-purple-700' : 'bg-gradient-to-br from-emerald-500 via-emerald-500 to-teal-600'}`}>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8 relative z-10">
          <div className="flex flex-col gap-1.5 text-white">
            ...
            {!isPurchaseMode && canViewTotalValue && (
               <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 mt-4">
                  <div className="bg-white/10 backdrop-blur-md rounded-2xl px-4 py-2 border border-white/20">
                     <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest opacity-80 block mb-0.5">Materiais Usinagem</span>
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-3">
            {canPurchase && !canAddProduct && !canEditProduct && !canDeleteProduct && (
              <Button
                variant="outline"
            ...
            {(canAddProduct || canEditProduct || canDeleteProduct) && canPurchase && (
              <Button
                variant="secondary"
      ...
      <div className="flex-1 -mt-10 sm:-mt-12 relative z-20 px-3 sm:px-4 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* Formulário - Só aparece se puder adicionar ou editar */}
        {(canAddProduct || canEditProduct) && !isPurchaseMode && (
          <>
            <div 
            ...
            <div className={`fixed inset-x-0 bottom-0 z-[999] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${showMobileForm ? "translate-y-0" : "translate-y-full"} lg:static lg:translate-y-0 lg:z-auto lg:block lg:col-span-4 xl:col-span-4 lg:h-fit lg:sticky lg:top-6`}>
              <div className={`bg-white dark:bg-slate-900 rounded-t-[40px] lg:rounded-[32px] p-6 pb-28 lg:p-5 lg:pb-5 shadow-[0_-20px_60px_-15px_rgba(0,0,0,0.1)] lg:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] border-t border-slate-100/80 lg:border dark:border-slate-800 flex flex-col max-h-[90vh] lg:max-h-none overflow-y-auto custom-scrollbar transition-all duration-500 ${editingProduct ? "ring-0 lg:ring-2 ring-amber-400/50" : ""}`}>
                ...
                {/* Se não pode criar novos produtos e não está editando, oculta os campos e exibe mensagem amigável */}
                {!canAddProduct && !editingProduct ? (
                   <div className="flex flex-col items-center justify-center h-full text-slate-400 p-8 text-center min-h-[300px]">
                      ...
                      <p className="font-bold text-lg text-slate-600 dark:text-slate-300">Modo de Edição</p>
                      <p className="font-medium text-sm mt-2">Você tem permissão apenas para editar itens. Selecione um produto na lista ao lado.</p>
                   </div>
                ) : (
                  <>
        ...
        {canAddProduct && !isPurchaseMode && !showMobileForm && (
            <button
        ...
        <div className={`${(canAddProduct || canEditProduct) && !isPurchaseMode ? "lg:col-span-8 xl:col-span-8" : "lg:col-span-12"} flex flex-col h-full`}>
          ...
          <div className="flex-1 overflow-y-auto pr-1 sm:pr-3 custom-scrollbar pb-10">
            {isError ? (
              <div className="h-[40vh] sm:h-[50vh] flex flex-col items-center justify-center text-red-500 bg-red-50 dark:bg-red-900/10 rounded-[40px] border border-dashed border-red-200 dark:border-red-800 mx-1 p-6 text-center">
                <AlertCircle className="h-14 w-14 mb-4 opacity-50" />
                <p className="font-black text-2xl tracking-tight mb-2">Erro de Comunicação</p>
                <p className="font-medium text-red-400 max-w-md">
                  {(error as any)?.response?.data?.error || error?.message || "Erro desconhecido ao carregar produtos."}
                </p>
              </div>
            ) : isLoading ? (
              ...
              <div className={`grid grid-cols-1 gap-5 sm:gap-6 ${(canAddProduct || canEditProduct) && !isPurchaseMode ? "md:grid-cols-2 xl:grid-cols-2" : "md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}>
                {paginatedProducts.map((product: any) => (
                  <ProductCard 
                    ...
                    isJustAdded={sessionAddedIds.includes(product.id)} 
                    onEdit={handleEdit}
                    ...
                    canViewPrice={canViewTotalValue}
      ...
      {/* ♻️ Modal de Reativação de Produto Fantasma */}
      {(canAddProduct || canEditProduct) && (
        <AlertDialog open={!!skuToReactivate} onOpenChange={(open) => !open && setSkuToReactivate(null)}>
          <AlertDialogContent className="w-full max-w-[100vw] sm:max-w-sm !rounded-b-none !rounded-t-[40px] sm:!rounded-[40px] p-8 sm:p-10 text-center bg-white dark:bg-slate-900 border-none shadow-[0_-20px_80px_-15px_rgba(0,0,0,0.3)] sm:shadow-2xl !top-auto !bottom-0 !translate-y-0 sm:!top-[50%] sm:!translate-y-[-50%] absolute sm:fixed mx-auto z-[1000]">
            <div className="w-14 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full mx-auto mb-8 sm:hidden"></div>
            <div className="mx-auto bg-amber-50 text-amber-500 h-20 w-20 rounded-[28px] flex items-center justify-center mb-6 shadow-inner">
              <AlertCircle className="h-10 w-10" strokeWidth={2.5} />
            </div>
            <AlertDialogHeader>
              <AlertDialogTitle className="text-3xl font-black text-center tracking-tight text-slate-900 dark:text-white">Produto Arquivado!</AlertDialogTitle>
              <AlertDialogDescription className="text-base text-center font-semibold mt-3 text-slate-500">
                O código SKU <strong className="text-slate-800 dark:text-slate-200">{skuToReactivate}</strong> já pertence a um produto que foi arquivado (inativo). Deseja reativar este produto para voltar a usá-lo?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="flex flex-col sm:flex-row gap-3 mt-10">
              <AlertDialogCancel className="rounded-[24px] h-16 flex-1 mt-0 font-bold border-none bg-slate-100 hover:bg-slate-200 text-[17px] dark:bg-slate-800 dark:text-slate-300">Cancelar</AlertDialogCancel>
              <AlertDialogAction onClick={() => skuToReactivate && reactivateMutation.mutate(skuToReactivate)} className="rounded-[24px] h-16 flex-1 bg-amber-500 hover:bg-amber-600 font-bold shadow-xl shadow-amber-500/20 text-[17px] text-white">Sim, Reativar</AlertDialogAction>
            </div>
          </AlertDialogContent>
        </AlertDialog>
      )}
